...
栈级缓存不是固定白名单。Java/Maven、Node/前端、Python、小程序只是当前已沉淀的高频落地规则；本轮实际读取哪个部分由当前工具动作、触碰文件、最近配置、命令、日志、diff 和 active cache path 决定。第三方中转、未知 wrapper、未列名语言或自定义构建脚本进入工具链节点时，先按 `06#当前工具与技术栈动态追加门禁` 确认项目根、命令来源、缓存路径和验证边界，再复用本文件最贴近的栈级规则；确实没有匹配栈时，只执行公共 active cache path、locale/encoding、`.codex/` 忽略和最轻量非交互验证，不为补齐 schema 扫描无关技术栈。
...
5. 写入缓存：
   - 写入 `mavenRoot`、`modulePath`、`artifactId`、Java 版本约束、实际 JDK home/version、Maven/wrapper 路径、localRepository、`.mvn` 配置摘要、`project.build.sourceEncoding`/resources 编码证据、选定命令和验证命令。
   - Java 目标版本和 Maven 实际运行 JDK 要分开记录，避免把“能运行 Maven 的 JDK”误当成“项目目标 JDK”。

Maven/Java 命令失败后的重算：

- 如果失败信息涉及 JDK 版本、`release/source/target`、`JAVA_HOME`、Maven 版本、wrapper、本地仓库、插件版本、依赖解析、模块路径、`-pl` 选择、toolchain、surefire/failsafe、`MalformedInputException`、`UnmappableCharacterException`、`file.encoding`、resources filtering 或中文资源乱码，必须重新读取 `pom.xml`、`.mvn/*`、wrapper、IDE 配置、项目编码配置和本机候选环境。
...
3. 读取语法、缩进和格式规范文件：
   ...
   - 编码/展示配置：HTML 或模板中的 `charset`、框架 public/index 模板、SSR 响应头配置、i18n 资源文件、构建工具处理文本资源的插件配置；命中中文乱码时按 `01#跨技术栈编码与中文乱码门禁` 追加检查。
...
5. 写入缓存：
   - 写入 `packageRoot`、配置文件、Python 版本约束、解释器路径/version、管理器名称/path/version、虚拟环境路径、lockfile、选定命令、验证命令，以及命中中文/文件 IO 风险时的源文件编码声明、fixture 编码、locale 或 `encoding=` 依据。
   - 只写项目声明和已验证结果，不写 pip 安装日志、私有 index token、`.env` 内容或临时失败详情。

Python 命令失败后的重算：

- 如果失败信息涉及 Python 版本、虚拟环境、依赖解析、lockfile、管理器、模块导入、工具命令不存在、tox/nox env、ABI/wheel、`Requires-Python`、`UnicodeDecodeError`、`UnicodeEncodeError`、locale 或 fixture 文件编码，必须重新读取 Python 配置、锁文件、虚拟环境、编码依据和本机候选环境。
...
1. 定位小程序项目和框架：
   ...
   - 命中中文乱码、平台资源乱码或页面文案异常时，读取页面模板、i18n/字典资源、构建输出编码、Node 前端规范文件和平台配置中的 charset/locale 相关证据。
2. 读取 Node 和构建约束：
