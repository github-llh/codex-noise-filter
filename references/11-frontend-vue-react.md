...
先执行 `01-global-engineering-rules.md#跨技术栈环境与命令`，再按 `06-environment-discovery.md#node前端环境缓存` 读取或更新前端环境缓存。
...
先执行 `01-global-engineering-rules.md#跨技术栈环境与命令` 和 `06-environment-discovery.md#node前端环境缓存`，再选择 Vue/React 项目脚本。
