...
- 包管理器和 Node 版本优先级来自项目文件：`packageManager`、`pnpm-lock.yaml`、`yarn.lock`、`package-lock.json`、`bun.lockb`、`bun.lock`、`.npmrc`、`.yarnrc.yml`、`.nvmrc`、`.node-version`、`.tool-versions`、Volta 配置，以及当前机器 nvm/fnm/asdf/corepack 证据。
...
- 选择 Node 版本时必须参考 `engines.node`、`.nvmrc`、`.node-version`、`.tool-versions`、Volta 和构建工具版本要求；存在 nvm 时，按项目版本声明进入 nvm 上下文后验证 `node -v` 和包管理器版本，并把原始声明、解析版本、`NVM_DIR`/`nvm.sh` 和验证命令写入 active cache path；本机默认 Node 不匹配时，继续查找已安装候选并更新 active cache path。
- `npm`、`pnpm`、`yarn`、`bun` 是包管理器；`npx`、`pnpm dlx`、`yarn dlx` 是命令 runner，必须记录但不能误判为包管理器。
- active cache path 已记录同一 `packageJson`、lockfile、Node 约束、版本来源文件、包管理器、selected command 和 `frontendQuality` 配置文件集合，且文件 mtime/size/hash 仍匹配时，直接复用；只要规范文件、Node 版本声明或 `package.json` scripts/dependencies 变化，必须自动更新缓存。
...
- 只修触碰范围和已闭环调用链的 lint/type 问题；历史全仓问题记录边界，不扩大重构。
