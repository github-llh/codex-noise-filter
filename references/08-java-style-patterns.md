...
本文件按需读取。只有任务涉及枚举、固定值、参数校验、去硬编码、重复赋值、判空、Optional、函数式风格、Lombok 时打开。
...
- 不按变量名前缀判断，而按业务语义判断：只要是固定来源、状态、类型、协议、模式、动作、结果、默认值、阈值、时间窗、开关等字符串/数字，在触碰范围内都要检查是否应沉淀为业务 Enum、配置属性或集中常量对象。
- `SOURCE_TYPE_*`、`STATUS_*`、`PROTOCOL_*`、`DEFAULT_*` 只是常见示例，不是完整范围。
