...
抽象必须减少真实重复，并让业务意图更清晰；不要为了“高级”而引入难读的反射、复杂泛型或过度函数式代码。三处以上相似逻辑默认需要考虑收敛。性能敏感路径谨慎使用反射型 Bean copy；优先编译期映射、显式 helper 或受约束泛型，让输入输出类型关系在编译期可见。
...
抽公共方法、公共类或泛型 helper 前，先按 `01-global-engineering-rules.md#跨技术栈抽象抽离时机` 判断是否满足稳定共同语义和正确依赖方向。少量字段复制、一次性校验或当前类内直观 setter 不必强抽；多类型转换、重复字段映射、策略分派或 handler 复用可以使用类型参数，但不要退化成 `Object`、原始集合或反射万能方法。
