...
- 低风险闭环成立：直接按 `00-index.md` 追加对应 reference 并修改，例如硬编码走 `01` + 技术栈规则，注释契约缺口走 `01` + 对应技术栈规则，类型契约缺口走前端 `11` 或小程序 `12`，抽象时机缺口先走 `01#跨技术栈抽象抽离时机` 再按技术栈落地，Java 枚举/配置/重复逻辑走 `08`，后端分层走 `07`，事务/并发走 `09`，前端组件、导出类型、api client 和状态走 `04`/`11`。
