...
本节不是用户提醒后才触发的纠错规则，而是所有续跑、恢复和跨轮编程任务的自动门禁。只要当前任务可能继承了上一轮判断，或当前工作区的 skill/reference 可能比上一轮更新，就不能沿用旧记忆、旧 Context Capsule、旧计划或上一轮已加载的 reference 结论。

自动触发状态：

- 上下文恢复、自动续跑、跨窗口继续、Global/Goal 下一轮、Plan 执行到下一步、用户中途插入新指令后继续主任务。
- 当前消息明显引用上一轮结论、上一轮代码、上一轮截图、上一轮 diff、上一轮“已符合/无需修改/后续建议”等判断，即使用户没有说“上个会话”。
