...
本节是文件写入前的内部门禁，不需要用户提醒。只要本 skill 流程进入新增、修改、删除、移动、重构、补丁修复或批量替换节点，就必须先判断写入方式，不能默认先打一整块补丁，失败后再反复解释“上下文有偏差”。
...
本节是 Git/worktree 相关内部门禁，不需要用户提醒。只要本 skill 流程进入修改、同步、拉取、提交、暂存、创建分支、切换分支、推送、PR、回滚、合并、rebase、reset、跨目录修改或用户提到 worktree/分支/GitHub/Codex 工作区，就必须先自动确认当前 worktree 和目标项目分支关系。
