...
读取窗口是 token 预算工具，不是规则判断边界。只要准备修改代码、判断是否同步治理旧代码、或评估强规则是否低风险闭环，就必须先确认当前已读内容是否覆盖了本次修改单元。

必须扩窗的触发信号：

- 已读窗口内出现方法、类、组件、SQL、配置块或函数体被截断，无法看到完整开始/结束边界。
- 已读窗口内出现硬编码、重复 if/set、重复映射、配置写死、分层错位、缺注释、安全边界缺口等强规则信号，但相关声明、常量、helper、调用方或被调方不在窗口内。
- 修改点引用了未读字段、常量、枚举、DTO/VO、props/emits、hook/composable、配置 key、SQL 片段、测试 fixture 或工具函数。
- 同一文件已有明显范式，例如相邻方法、同类字段、同类组件、同类测试或同类枚举，但当前窗口没有覆盖对照样例。
- 只读了 diff 附近几行，却需要判断“是否已有相同逻辑”“是否应抽常量/枚举”“是否 Controller 写了业务逻辑”“是否缺注释/校验/事务边界”。
- 用户明确指出“没读到”“读取行数限制”“漏判”“无法自动更改”等读取完整性问题。

扩读顺序：

1. 先定位符号边界：用 IDE 符号、语言服务器、结构搜索、`rg`、`ctags`、AST/语法工具或局部括号匹配确认类、方法、组件、函数、SQL mapper、配置块的起止行。
2. 读取本次修改符号的完整体；如果完整体超过常规窗口，分段读取同一符号，不用一次性读全文件。
3. 读取同一文件的关键声明区：imports、字段、常量、枚举、注解、props/emits、类型定义、配置对象、路由/页面声明。
4. 读取直接调用点和被调用 helper 的最小窗口；若调用链已在其他文件，按调用链规则继续最小扩读。
5. 读取同文件相邻 1 到 3 个同类范式，用来判断项目风格、命名、注释、Lombok、校验、错误处理和抽象方式。
6. 若强规则判断仍不闭环，再用 `rg` 精确搜索同名常量、魔法值、重复片段、方法名、配置 key 或状态值，只读取命中的最小窗口。

不能做的事：

- 不能因为默认窗口是 200 到 300 行，就在未读完整方法/组件/类声明区时断言“没有明显问题”。
- 不能用“未读到”作为跳过低风险局部修复的理由；应先扩读能证明闭环的最小区间。
...
- 若文件过大、语法不完整、生成文件、二进制/压缩产物或工具无法定位符号边界，说明未读边界和保守处理策略；不得把未验证区域当成已检查。
