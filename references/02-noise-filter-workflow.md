...
不要把 skill 启用依赖在写出 skill 名、完整描述或固定提示词上。只要输入或上下文里有代码、diff、报错日志、异常堆栈、构建输出、测试输出、IDE 截图、路径、文件名、类名、方法名、配置文件、命令行片段或项目结构，就按编程任务处理，并通过 `00-index.md` 路由到必要 reference。
...
- 允许改动范围。
- 禁止改动项。
- 已发现的强规则违背：仅记录触碰范围、已闭环调用链或为完成任务必须读取的相关文件内的问题；低风险闭环项进入本轮同步处理，高风险或语义不闭环项记录为边界。
- 根因与调用链状态：原始症状、可复现信号、疑似根因、已确认入口、数据来源、调用方、被调方、配置/路由/任务/消息入口、持久化/缓存/事务/外部副作用和仍未追清的断点。
...
- 预期产物。
- 预计读取预算：文件数上限与行数窗口；预算只约束读取顺序和输出摘要，不约束根因追踪、调用链闭环或验证覆盖。
...
2. 读取：按 `00-index.md` 读取必要 reference、目标文件、同类范式和必要配置。
3. 确认根因与调用链：从症状向上追到触发入口和错误数据来源，再向下确认被调方、配置路由、持久化/缓存/消息/异步/外部副作用、影响面、回滚点和强规则同步项。
4. 修改：执行最小补丁，记录已写入文件、未写入文件、策略切换和回滚方式。
5. 验证：执行覆盖原始问题、触碰范围和受影响调用链的最小充分验证，记录命令、结果、失败分类和未验证边界。
...
| 读取与调用链 | 定位到真实文件、准备修改、准备重构、准备删除、准备验证已有结论 | 按 `00-index.md` 读取必要 reference；按 `13` 扩读完整语义单元；从症状反向追到触发入口和错误数据来源，再确认调用方、被调方、配置/路由/任务/消息入口、持久化/缓存/事务/外部副作用、影响面和回滚点 | 只凭片段、截图、旧 diff、直接调用点或第三方摘要修改 |
...
| 编码与乱码 | 看到中文、非 ASCII 文案、乱码字符、`encoding`/`charset`、`UTF-8`/`GBK`、控制台/编译/页面输出乱码、跨平台文件名或终端输出异常 | 按 `01#跨技术栈编码与中文乱码门禁` 确认文件编码、项目编码配置、终端 locale、编译/构建 charset、前端 meta/header 和验证方式；修复后用最小充分验证命令或文件读取确认 | 把乱码当展示问题略过；用个人编辑器默认编码覆盖项目规则 |
...
| 验证 | 写入后、第三方声称已验证、工具链失败后、准备交付前 | 按 `18` 选择覆盖触碰范围和调用链风险的最小充分非交互验证；判断 root/workspace、命令、环境缓存、原始问题覆盖、失败归因和 diff review；无法运行时说明缺口 | 不跑验证也不说明；只跑与根因无关的轻量命令；把外层工具未知 root 的结果当通过 |
...
- `rootCause`：排障、修复或失败诊断任务已记录原始症状、关键错误、复现/不可复现证据、最近变化点、单一当前假设和支持/反驳证据；未定位根因时不得写入修复补丁，只能补证据或诊断。
- `callChain`：已确认触发入口、错误数据来源、直接调用方、间接调用方、被调方、配置/路由/任务/消息入口、持久化/缓存/事务/权限/外部副作用、影响面、验证映射和回滚点；未闭环时不得写入。
...
- `validation`：已执行覆盖原始问题、触碰范围和受影响调用链的最小充分验证，或已说明无法验证原因和未覆盖边界。
...
- 若准备写入但缺少 `scope`、`rootCause` 或 `callChain`，停止写入，补读当前文件、diff、配置入口、调用链和错误数据来源；不能用“改动很小”替代根因闭环。
- 若已写入但缺少 `localAlignment`，先回扫同一语义单元和已闭环调用链，不直接进入最终回复。
...
- 若准备交付但缺少 `validation`，先执行最小充分验证；无法执行时说明具体缺口，不能把外层 Build 文案或与根因无关的轻量命令当作已验证。
...
1. 读取 `00-index.md` 并选择必要 reference。
2. 确认触碰范围和禁止触碰范围。
3. 确认根因、触发入口、错误数据来源、完整调用链、影响面和回滚点。
...
8. 修改后执行对应自检与最小充分验证，并确认验证命令覆盖原始问题、本次触碰范围和受影响调用链。
...
3. 只要恢复出的任务涉及代码读取、修改、构建、测试、lint、format、typecheck、调试、重构、注释契约、魔法值、常量、类型、环境缓存或验证，就内部追加 `00-index.md` 路由出的必要 reference。
...
7. 若任意第三方调用、agent、wrapper 或路由层已执行格式化、lint、typecheck、构建或测试，本 skill 仍要判断命令是否属于正确 root/workspace、是否使用 active cache path、是否覆盖原始问题、触碰范围和受影响调用链；不满足时补做最小充分验证或说明无法验证。
...
- `validation`: 最小充分验证项、覆盖的原始问题和调用链风险，以及不做浏览器、模拟器、真机、外部服务或桌面操作验证的边界。
...
- 用最新规则重新评估当前任务证据、触碰范围、已闭环调用链和为完成任务必须读取的相关文件，不把评估对象写死为截图、diff 或代码片段。
...
当代码片段、IDE 截图、diff、已读文件或调用链确认过程中打开的相关文件中已经出现本 skill 明确治理的问题时，不能把“最小改动范围”理解成“不处理”。先判断是否能在当前触碰范围和已闭环调用链内形成低风险闭环；能闭环就写入当前任务清单和任务胶囊，并直接进入对应 reference 做局部对齐，不能闭环才只诊断和记录风险。
...
- 编码与中文乱码缺口：已读文件、工具输出、页面文案、日志、配置、终端输出、编译产物或第三方转发载荷中出现中文乱码、替换字符、编码声明冲突、GBK/UTF-8 混用、跨平台文件名异常或编译器 source encoding 缺失，却没有确认项目编码、终端 locale、构建 charset、前端 `charset`/响应头和最小充分验证方式，也算未闭环。
...
- 真实文件和主修改点已定位，问题在当前类/方法/组件或已闭环调用链内。
- 触发入口、错误数据来源、直接/间接调用方、被调方和配置入口清楚，影响面主要停留在同一业务域或同一模块。
- 预计只涉及少量文件，不需要跨多个子系统、迁移数据、改公共协议或引入新依赖。
- 能保持 API、DTO、数据库字段、权限、路由、序列化值、外部协议和业务语义兼容。
- 有覆盖原始问题和受影响链路的最小充分验证路径，例如语法检查、编译、构建、lint/type check 或目标测试；默认不要求页面、接口、模拟器、真机、外部服务或人工点击等操作验证。
...
- 问题明显但属于触碰范围外：不扩大成全模块重构，只记录风险和建议；如果同类问题已经落在本次已闭环调用链或为完成任务必须读取的相关文件内，不能当成触碰范围外跳过，必须写入任务胶囊评估。
- 出现“参考这种方式”这类模式迁移意图时，把图或片段当模式信号，不把示例中的具体类名、字段名或文件路径写死成唯一规则。
- 出现字段截图、注解描述或片段中的固定取值时，把它当“同类固定业务取值治理”信号；先在真实目标文件和已闭环调用链中检索现有枚举/字典/生成类型，不把截图里的具体字段名、枚举项或业务名写死到 skill 规则里。
...
预算规则只服务于证据读取顺序：先定位、再扩读、再摘要。它不能把“未读到”伪装成“不存在”，也不能压过根因追踪、调用链闭环、局部对齐和验证覆盖。若预算压力与一次性解决问题冲突，先输出/刷新 Capsule、保留证据锚点，再继续补齐闭环所需的最小证据。
...
涉及排障、改代码、重构、重命名、移动、删除、验证第三方结论或修复上次未彻底解决的问题前，必须先完成根因追踪与调用链确认。这里的“调用链”不是只找一个直接调用方；它必须说明问题从哪里触发、错误数据从哪里来、影响会流向哪里、验证要覆盖哪里。

根因追踪顺序：

1. 记录原始症状：用户问题、失败命令、关键错误、堆栈、日志、截图路径、接口/页面/任务名和最近变化点。
2. 定位直接失败点：抛错行、失败断言、异常返回、错误状态、乱码输出、性能瓶颈或错误数据出现的位置。
3. 反向追溯来源：谁传入了错误值、谁触发了当前方法/组件/脚本/任务、配置或路由如何进入、默认值/缓存/环境变量/消息/外部回调是否参与。
4. 继续向上直到入口：Controller/API、页面事件、CLI 参数、定时任务、消息消费者、hook、测试 fixture、配置加载、第三方转发或用户操作入口；不能只停在中间 helper。
5. 向下确认影响：被调 Service/DAO/mapper/SQL、组件/hook/store/api client、文件 IO、缓存 key、事务边界、消息投递、外部 API、副作用和错误处理。
6. 对照同类正常路径：读取同模块 1 到 3 个工作范式、同类配置或历史测试，确认当前问题是个例、模式偏差还是公共抽象缺口。
7. 形成单一当前假设：用一句话写明“我认为根因是 X，因为证据 Y；还未确认 Z”。未形成可检验假设时，只能继续补证据，不能写修复补丁。

调用链闭环必须包含：

- 主修改点：文件、类/函数/组件/配置块、行为边界、输入输出。
- 上游入口：直接调用方、间接调用方、路由/配置/脚本/任务/消息/测试入口，以及每个入口为何会到达本逻辑。
- 下游依赖：被调用 helper、Service、DAO/Repository/Mapper/SQL、API client、store、hook/composable、缓存、消息、事务、文件和外部系统。
- 数据与状态：关键字段、DTO/VO/Entity/schema、状态值、枚举/字典、配置 key、默认值、缓存 key、权限/租户/审计上下文。
- 影响面：API/DTO/数据库字段/权限/事务/缓存/消息/路由/序列化值/配置/前端展示/测试 fixture 是否变化。
- 验证映射：哪条命令或测试能证明原始症状被覆盖，哪条命令能证明受影响链路未破坏，哪些边界无法自动验证。
- 回滚点：最小补丁、涉及文件清单、兼容保留项和不可回滚副作用。

多组件或证据不足时，先加诊断证据而不是猜修：

- 在各组件边界记录输入、输出、配置、环境和状态，不把大量日志长期保留，只保留能定位断点的关键字段。
- 对深层堆栈或错误值，必要时临时加入可移除的栈追踪/调试输出，运行一次确认断点后再决定是否保留正式防御性日志或校验。
- 若同一问题已经改过仍复发，先比较当前 diff、历史改动、失败路径和验证缺口；不得直接在症状点再叠一层补丁。

未完整确认根因与调用链时，不得进入修改步骤；先回报缺口并继续追链。只有语法错误、拼写错误、明显引用缺失等可由当前完整语义单元直接证明的低风险问题，才可把根因闭环压缩到该语义单元内，但仍要说明为什么不需要继续向上追。
...
11. 验证命令是否覆盖原始问题、触碰范围和受影响调用链；无法验证时是否说明原因。
...
失败处理的目标是快速切换策略，不是用同一错误假设反复重试。每次失败后先判断失败类型、已写入状态、根因假设是否被证伪、调用链是否漏读和可回滚点；如果工具返回“整体拒绝、没有写入”，不得把它当成已经部分修改。
...
3. 说明错误假设和当前文件是否已写入。
4. 只做一次针对根因的最小修复；若失败类型是补丁上下文不匹配，必须先重新打开目标文件关键段，再改用更小上下文的逐文件精准补丁。

同类失败第二次或出现“补丁整体拒绝且没有写入”：

1. 停止用原补丁或同一上下文继续重试。
2. 回到根因追踪与调用链确认：复核原始症状、触发入口、错误数据来源、直接/间接调用方、配置/路由/消息/缓存/事务/外部副作用和验证缺口，更新 `doNotRetry`。
3. 按文件逐个重新读取关键段，确认当前原文、目标锚点、预期新内容和不变项。
4. 根据真实差异选择策略：小范围文本一致时用逐个精准 patch；锚点稳定但上下文漂移时用 IDE/结构化替换；整段语义单元已明显变化时替换完整最小语义单元；无法证明边界时只诊断并说明缺口。
5. 每次只处理一个文件或一个语义单元，成功后立即重新读取或 `git diff` 校验，不把多个不确定文件合成一个大补丁。
6. 将策略切换、已成功文件、未写入文件、回滚点和下一步写入任务胶囊。

同一问题第三次修复仍失败、同一假设第三次被证伪，或每次修复都在不同组件暴露新耦合：

1. 停止继续叠补丁。
2. 判断是否是方案、抽象、架构、数据模型、事务边界、环境假设或外部契约错误，而不是局部代码漏改。
3. 输出当前证据链、已经排除的假设、仍缺的调用链或验证证据、可能的更大边界和需要用户确认的决策。
4. 只有用户明确授权或补齐证据后，才继续做下一轮设计或修复。
