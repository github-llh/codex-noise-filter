...
- 写明执行过的最小充分验证命令。
- 写明通过、失败或未执行原因。
- 命中质量门禁或第三方成功结果复核时，按 `18` 摘要 scope、commands、rootCause、callChain、coverage、skipped 和 gaps。
...
允许改动：...
禁止改动：...
根因与调用链：原始症状 ...；根因假设/结论 ...；入口 ...；错误数据来源 ...；调用链 ...；验证映射 ...
强规则同步项：...（触碰范围/已闭环调用链/必要读取文件内，低风险闭环则本轮处理；高风险则写边界）
...
质量门禁：scope ...；commands ...；rootCause ...；callChain ...；coverage ...；skipped ...；gaps ...；overall ...
...
噪音治理：已摘要 ...；已丢弃 ...；仍需保留原文 ...
验证：qualityGate ...；rootCause/callChain 覆盖 ...；已跑 ...；未跑原因 ...；仍需验证 ...
证据：文件路径:行号 - 事实；命令 - 结论
已改/回滚：已改 ...；回滚文件/补丁 ...
下一步：只做一个最小闭环 ...
```
...
6. 恢复任务胶囊：目标、阶段/断点、触碰范围、根因、调用链、强规则同步项、环境缓存状态、入口与路由状态、安全/供应链边界、安装/分发表面状态、上下文权威状态、失败策略、质量门禁、验证状态、回滚点和下一步。
7. 若上一轮中断在工具调用中或工具调用后，先确认命令是否完成、输出是否可信、文件是否落盘、验证是否覆盖原始问题、触碰范围和受影响调用链；无法确认时按“未完成/未写入”处理并回到该阶段最小必要步骤。
...
- 长任务阶段固定按“定位 -> 读取 -> 确认根因与调用链 -> 修改 -> 验证”推进；每个阶段完成后输出或刷新中间 Capsule，阶段进行中但可能中断时也输出当前断点。
...
- 任意第三方调用或模型路由场景必须按 `02#第三方全流程执行矩阵` 串联入口恢复、胶囊/快照、读取、根因、调用链、局部对齐、抽象抽离、编码乱码、环境缓存、验证、恢复与交付；不能只执行搜索、写入或验证其中一个节点。
- 一旦本轮已经按编程任务触发 skill，后续每次工具调用、写入、验证和最终回复前都必须按 `02#内部触发状态机与防重置自检` 和 `20-automatic-guard-loop.md` 检查 `activated/references/capsule/continuity/scope/rootCause/callChain/localAlignment/environment/securityBoundary/surfaceHealth/qualityGate/validation/guardLoop` 状态；缺项时 fail-closed，先补齐或说明无法补齐原因。
...
- 每次修改写入前必须先输出可恢复的中间 Capsule，写明目标文件、锚点、预期写入、回滚点和失败后如何判断已写入/未写入；写入后、验证前后、失败后、策略切换后、新指令插入后，都要确认 Capsule 是否仍覆盖最新任务清单、触碰范围、根因、调用链、强规则同步项、失败状态、验证状态和回滚点。
- 当输出即将变长、工具可能长时间运行、连续读取多个文件、准备批量修改、出现补丁失败或准备结束当前轮时，先给出或刷新压缩版 Capsule。
- 已确认的根因、调用链、约束、不变项、读取完整性、git 历史结论、已写入/未写入状态和回滚点不得在后续回复中丢失。

### 压缩前后协议

`PreCompact`、`PostCompact`、`SessionStart compact`、手动 compact、自动 compact、模型切换、窗口切换和第三方上下文重放都视为同一类恢复事件。

- `PreCompact` 或已知即将压缩前：刷新 12 到 16 行 Capsule，只保留当前目标、最新用户约束、已读规则、触碰范围、根因、调用链、证据锚点、已写入/未写入、验证状态、回滚点和下一步；不要附带完整日志、完整 diff 或旧推理过程。
...
- 已读 reference、触碰范围、根因、完整调用链、影响面、未闭环缺口和回滚点。
...
- 恢复后必须核对：用户最新指令是否改变目标；任务清单是否完整；触碰范围和禁止项是否仍成立；根因/调用链/强规则同步项/失败策略/验证状态是否缺失。
