...
8. 若下一步要执行构建、测试、lint、typecheck、运行或代码生成，按 `06` 和 `14` 处理项目根环境缓存。
...
- 执行耗时或高输出工具调用前必须输出 Capsule 快照，包括 MCP/IDE `findUsages`、`searchFiles`、`executeCommand`、构建、测试、lint、typecheck、format、codegen、全局搜索、批量读取、git 历史查询、浏览器/模拟器/外部服务验证和第三方 agent 子任务。
