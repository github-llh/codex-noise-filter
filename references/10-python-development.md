...
本文件按需读取。是否打开由本 skill 根据文件证据、项目配置、命令节点、报错输出、触碰范围和调用链内部判定；命中 Python 语法、脚本、服务、包管理、虚拟环境、依赖、运行、测试、lint/format、类型检查或 Python 性能时必须打开，外部点名不作为前提。文件归属、环境命令、验证策略、安全边界、硬编码、重复逻辑和注释的跨技术栈判断先按 `01-global-engineering-rules.md` 执行；通用不可绕过门禁、调用链和既有代码局部对齐仍按 `02-noise-filter-workflow.md` 执行；环境路径发现见 `06-environment-discovery.md`。
...
先执行 `01-global-engineering-rules.md#跨技术栈文件归属与依赖边界`，再按 Python 包结构落地。
...
先执行 `01-global-engineering-rules.md#跨技术栈环境与命令`，再按 `06-environment-discovery.md#跨系统缓存文件命名` 解析 active cache path，并按 `14-environment-cache-by-stack.md#python-环境缓存` 读取/复用 Python 版本、虚拟环境、管理器和项目命令。

- 环境发现顺序：项目缓存、`pyproject.toml`/锁文件/requirements/tox/nox/版本文件、`.venv`、IDE/项目配置、shell 环境、`which python/python3/uv/poetry/pytest`，最后才查本机候选。
- 缓存命中同一 `packageRoot`、配置文件、lockfile、Python 版本约束和管理器时直接复用；不匹配时自动重算，不要求手动指定解释器。
- 优先复用项目已有工具链：
...
先执行 `01-global-engineering-rules.md#跨技术栈环境与命令`，再选择 Python 项目命令。

- 优先使用项目文档、Makefile、justfile、tox、nox、CI、`pyproject.toml` scripts 或现有 README 中的命令。
- 命令执行前必须使用 `06` 缓存里匹配的解释器、虚拟环境和管理器；不要凭经验直接调用系统 `python`、全局 `pytest` 或全局 `ruff`。
...
- 运行/导入/lint/type check/test 失败且疑似 Python 版本、虚拟环境、依赖、lockfile、管理器或工具命令不匹配时，必须重跑 `14-environment-cache-by-stack.md#python-环境缓存`，更新缓存后用同一目标命令重试一次。

## 测试与验证

先执行 `01-global-engineering-rules.md#跨技术栈验证策略`，再选择 Python 定向验证。

- 默认验收以语法、导入、类型检查或 lint 通过为准，不默认运行服务、脚本主流程、数据库/网络/队列集成、外部 API 或操作性验证。
- 验证命令优先来自缓存中的 `selectedCommand` 或项目已有命令；缓存缺失时先建立 Python 环境缓存再执行。
- 优先识别项目测试框架：pytest、unittest、tox、nox、coverage、hypothesis、pytest-asyncio、pytest-django、pytest-cov。
- pytest 项目只有在当前任务目标是测试修复、需要复现 bug，或编译/语法无法覆盖关键风险时，才运行最小目标：`python -m pytest path/to/test_file.py::test_name`；需要全量时再跑目录或完整套件。
- unittest 项目只有在需要测试验证时，才运行 `python -m unittest` 或项目已有命令。
...
- 涉及 bug 修复时，优先保证语法/导入/类型或 lint 通过；是否补定向测试取决于项目已有低成本测试能力和当前 bug 风险，不能补测试时说明原因。
...
- 类型检查只对触碰范围和已闭环调用链做必要补强；不要为了消除一个错误大面积重写模型、协议或 API。
...
先执行 `01-global-engineering-rules.md#跨技术栈安全与外部边界`，再按 Python 配置系统落地。
