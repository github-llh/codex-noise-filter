...
先执行 `01-global-engineering-rules.md#跨技术栈环境与命令`，再按 `06-environment-discovery.md#python-环境缓存` 读取/复用 `.codex/local-environment.json`、Python 版本、虚拟环境、管理器和项目命令。
...
- 运行/导入/lint/type check/test 失败且疑似 Python 版本、虚拟环境、依赖、lockfile、管理器或工具命令不匹配时，必须重跑 `06-environment-discovery.md#python-环境缓存`，更新缓存后用同一目标命令重试一次。
