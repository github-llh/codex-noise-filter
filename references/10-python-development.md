...
- 缓存命中同一 `packageRoot`、配置文件、lockfile、Python 版本约束和管理器时直接复用；不匹配时自动重算，不要求用户手动指定解释器。
- 优先复用项目已有工具链：
...
- pytest 项目只有在用户明确要求、任务本身是测试修复、或需要复现 bug 时，才运行最小目标：`python -m pytest path/to/test_file.py::test_name`；需要全量时再跑目录或完整套件。
...
- 涉及 bug 修复时，优先保证语法/导入/类型或 lint 通过；是否补定向测试取决于用户要求、项目已有低成本测试能力和当前 bug 风险，不能补测试时说明原因。
