...
- 新增公共层前先按 `01-global-engineering-rules.md#跨技术栈抽象抽离时机` 判断共同语义、变化点稳定性、依赖方向、契约风险和验证路径；不能因为两段代码相似就直接抽到 `common` 包。
- 只在多实现、多业务类型、多状态处理、稳定转换链路、复用组装逻辑或跨接口共享规则已经明确时，抽公共接口、抽象类、领域组件、策略、assembler/converter 或 helper。
- 后端公共抽象允许使用受约束泛型保留输入输出类型关系，例如 converter、assembler、strategy、handler、repository adapter 或 response wrapper；泛型参数必须能从调用点看出业务含义，并尽量通过接口、上界、枚举或领域类型约束。
- 不用裸 `Object`、原始 `List/Map`、反射型通用处理、复杂嵌套泛型或万能 base class 逃避建模；如果泛型让事务、权限、租户、审计、异常或日志边界变模糊，就保持显式业务方法或按业务域拆分。
- 抽象后必须保持旧接口、DTO、数据库字段、序列化值、权限和事务语义兼容；无法用最小编译/测试验证闭环时，只做诊断和局部收敛，不做跨模块公共抽象。
...
- VO/Response/DTO、OpenAPI/Swagger 注解或字段说明列出固定 code/label 时，先确认项目是否已有同业务语义的 Enum、字典或生成类型；已有则字段注释、入参校验、转换和业务判断都要指向该定义。公共契约因兼容仍用 `String` 时，也要在业务层使用枚举 code/lookup，不在实现里裸写同一组 code。
