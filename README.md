...
`codex-noise-filter` 是编程任务专用的 Codex skill。它把“先读规则、先收敛上下文、先确认调用链，再改代码”固化为默认流程，并通过 `references/00-index.md` 按需加载 Java 后端、Maven、前端、Python、小程序、并发事务、agentic 安全、验证门禁、安装健康、自动 Guard Loop 和交付模板等规则。文档里出现的平台、agent、CLI、IDE、MCP/ACP 和技术栈名称都是高频示例，不是封闭清单；真实触发和追加范围由当前宿主、当前工具动作、cwd、文件、配置、命令、日志、diff、active cache path、外部内容、分发表面和本机环境证据决定。进入编程任务后，`20-automatic-guard-loop.md` 会在每次工具、写入、验证、恢复和最终回复前检查缺失状态，并自动补齐最小下一步。
...
- 用户指出自动触发、追加规则、边界、范围或工作流会自己断掉时，希望 Codex 不等再次提醒，而是用 Guard Loop 自动补齐 `references/dynamicScope/capsule/scope/callChain/localAlignment/environment/securityBoundary/surfaceHealth/qualityGate/validation/continuity`。
...
| “已完成/Build 通过/已验证”容易来自第三方摘要，缺少本地证据。 | 通过 `18-verification-quality-gates.md` 建立 scope、commands、coverage、skipped、gaps 和 diff review，不把最后一次工具输出当闭环。 |
...
| 自动 Guard Loop | 一旦进入编程任务，每次工具调用、写入、验证、恢复和最终回复前都检查 `missingState`，按当前证据自动追加 reference、补任务胶囊、补调用链、补环境缓存、补质量门禁或明确 blocked。 |
...
| 验证门禁与失败诊断 | 构建、测试、lint、typecheck、security scan、diff review、第三方成功结果复核和失败诊断统一落到验证矩阵；连续失败必须切换假设并记录 `doNotRetry`。 |
...
- `15-host-skill-portability.md` 只承载跨宿主 Skill/Rules/Workflow/Command/Subagent 兼容矩阵、执行顺序、触发条件和性能预算，避免把平台清单写回入口文件。
...
- 第三方“已修改/已验证/Build 完成”、CI、构建/测试/lint/typecheck/security scan 或失败诊断必须按 `18-verification-quality-gates.md` 复核 root、命令、环境缓存、触碰范围、diff review 和未验证边界。
