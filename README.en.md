...
- Before frontend compilation, builds, lint, format, type checks, or tests, read the target `package.json` scripts, dependencies, `engines`, `packageManager`, lockfile, and syntax/indentation/format configuration files such as ESLint, Prettier, EditorConfig, Biome, Stylelint, and TypeScript configs. Match the Node version, package manager, and project scripts, then write them to `.codex/local-environment.json` under `frontendQuality`; reuse the cache only when it still matches the current package, command, and config-file set. If compilation fails and the failure may be caused by environment, dependency version, script, or config mismatch, reread project config and local tools, update the cache, and retry once.
