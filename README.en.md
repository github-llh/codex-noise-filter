...
| Smart window expansion | When editing code or judging strong-rule issues, local read windows are only the starting point; if the window is insufficient, expand to full methods/components/class declarations, fields/constants, direct calls, and nearby peer patterns to avoid misses caused by unread code. |
...
- Read windows are only an initial budget. Before editing code or judging strong rules, if the current window does not cover the full method, component, class declaration area, fields/constants, direct calls, or required nearby peer patterns, expand the read first instead of skipping local improvements because the code was not read.
