...
- Cases where automatic triggering, rule/scope/boundary expansion, or the workflow itself feels disconnected; Codex should fill `references/dynamicScope/capsule/scope/rootCause/callChain/localAlignment/environment/securityBoundary/surfaceHealth/qualityGate/validation/continuity` without waiting for another reminder.
- Signals that the task needs lower token usage, concise evidence, reproducible reasoning, narrower file reads, or preserved evidence chains. Lower token usage may optimize read order only; it must not reduce root-cause tracing, call-chain closure, or validation coverage.
...
| Without it | With it |
| --- | --- |
| After pasting an error log, you still have to explain "debug/fix this" repeatedly. | Infer a debugging task from logs, stack traces, paths, commands, and the current repository, then trace the root cause, trigger entry, and bad-data source before applying the smallest fix and validation. |
| Repository-wide scanning can consume context with unrelated files and logs. | Read `references/00-index.md` first and load only task-relevant references; reading less unrelated material does not mean tracing less necessary call chain. |
| Edits may ignore Controller/Service/DTO/Entity call-chain impact. | Confirm touched scope, root cause, trigger entry, call chain, and unaffected contracts before changes. |
...
| Unlisted platforms, agents, or new stacks may be misread as "outside the rules". | Names are only routing hints. Scope is added dynamically from tools, files, configs, commands, cache, and local environment evidence; unknown stacks still use cross-stack gates and the smallest sufficient validation. |
...
| "Done", "Build passed", or "validated" may come from a third-party summary without local evidence. | `18-verification-quality-gates.md` records scope, commands, rootCause, callChain, coverage, skipped checks, gaps, and diff review instead of treating the last tool output as closure. |
...
```text
I pasted a Maven BUILD FAILURE. Trace the root cause and call chain directly; if this repository has enough context, apply the smallest fix and rerun the smallest sufficient validation command.
```
...
```text
$codex-noise-filter use Maven multi-module root build rules to find the smallest sufficient validation command for this module.
```
...
| Non-bypass gates | New code, existing-code edits, Plan, Global/Goal, resumes, and cross-window work must confirm touched scope, root cause, call chains, and local alignment. |
| Root-cause and call-chain closure | For errors, failures, repeated fixes, or third-party result review, trace from symptom back to trigger entry and bad-data source, then confirm callers, callees, config/routing/task/message entries, persistence/cache/transaction/external side effects, and validation mapping. |
| Smart window expansion | When editing code or judging strong-rule issues, local read windows are only the starting point; automatically expand by each stack's syntax for Java, Python, Vue/React, Mini Programs, SQL, config, scripts, and tests to cover full semantic units, contracts, trigger entries, direct calls, and nearby peer patterns. |
...
| Strong-rule auto escalation | When the touched scope, closed call chain, or related files that must be read for the task already hit hardcoding, repeated logic, hardcoded config, layering mistakes, comment-contract gaps, or security-boundary gaps, judge whether a low-risk closure exists; write it into the task capsule and fix it directly when it does, and record risk only when it does not. |
| Automatic Guard Loop | After a coding task activates, every tool call, write, validation, recovery, and final response checks `missingState`, adds references from current evidence, and fills capsules, root cause, call chains, environment cache, quality gates, or blocked reasons before moving on. |
...
| Dynamic tool/stack scope | Third-party forwarding, unknown wrappers, unlisted platforms, or new stacks are not judged by a fixed list. Add the smallest references, environment cache checks, and smallest sufficient validation scope from the current host, tool action, files/configs/commands/logs/diffs, active cache path, and local environment evidence. |
...
| Verification gates and failure diagnosis | Builds, tests, lint, typecheck, security scans, diff review, third-party success claims, and failure diagnosis use one verification matrix; validation must cover the original issue, root cause, and affected call chain, and repeated failures must switch hypotheses and record `doNotRetry`. |
...
- `02-noise-filter-workflow.md` only contains cross-stack execution gates, context budgets, root-cause tracing, call-chain checks, and touched-scope alignment. Smart read expansion and Git history regression guards live in `13-read-expansion-and-history.md`, while stack-specific differences route to the matching reference files.
- `15-host-skill-portability.md` only contains the cross-host Skill/Rules/Workflow/Command/Subagent compatibility matrix, execution order, trigger conditions, and read-efficiency budget, so platform lists do not return to the entry file.
...
- Before editing, confirm target files, original symptom, root cause, trigger entry, bad-data source, minimal fix, and unaffected contracts.
- Confirm call chains, impact, and validation mapping before code changes.
...
- When edits affect behavior semantics, public contracts, historical compatibility, regression risk, or old-logic refactors, automatically read the smallest useful git history evidence with `git log`, `git blame`, `git show`, `git diff`, and `git log -S/-G`. Scope history reads to touched files, semantic units, key tokens, and closed call chains; do not scan the full repository history or use git commands that mutate the worktree.
- Minimal change is not a reason to ignore strong-rule hits. If hardcoding, repeated logic, hardcoded config, layering mistakes, or comment/security gaps are already inside the touched scope, closed call chain, or related files that must be read for the task, judge low-risk closure first, write it into the task capsule, and fix locally when it holds.
- Regardless of whether the work is new code, an existing-code edit, Plan, Global/Goal mode, auto-resume, context restoration, cross-window continuation, a local patch, or a follow-up fix, coding tasks must run through the skill index, touched-scope confirmation, root-cause tracing, call-chain confirmation, and local rule alignment.
...
- Third-party "changed/validated/Build complete" claims, CI, builds, tests, lint, typecheck, security scans, or failure diagnosis must be reviewed with `18-verification-quality-gates.md` against root, command, environment cache, root cause, call chain, touched scope, diff review, and uncovered gaps.
...
- Rules apply to both new code and existing-code edits. Every touched method, class, DTO, SQL, test, and closed call-chain segment must be locally aligned with the rules.
...
- When Chinese text, non-ASCII content, `encoding`, `charset`, UTF-8/GBK, locale, terminal output, page text, or build resources show mojibake or garbled output, confirm the project encoding source, active cache path, toolchain locale, and the smallest sufficient verification path instead of treating it as a third-party display issue.
...
- Python edits should get the smallest sufficient validation: syntax/import checks, targeted tests, and lint/format/type checks that cover the original issue, touched scope, and call chain. If validation cannot run, state why.
...
- If an existing cache still passes tool-availability validation and satisfies the current command, reuse it without searching local paths again.
...
- For long tasks, use the sequence: task capsule, root-cause and call-chain confirmation, minimal edit, smallest sufficient validation, Context Capsule.
...
- When a code snippet, IDE screenshot, or call-chain read exposes an obvious rule violation, locate the real file, root cause, and closed call chain first. If impact is shallow, file count is small, and compatibility is clear, write it into the task capsule, apply the matching reference, and fix it instead of only listing a follow-up.
