...
| Environment discovery | For builds, compilation, tests, runs, previews, or pre-release checks, read stack-specific project facts such as `pom.xml`, `pyproject.toml`, `package.json`, and Mini Program config, then validate and reuse `.codex/local-environment.<profile>.json`; legacy `.codex/local-environment.json` is only a compatibility fallback. If the cache does not satisfy the command or a failure looks environment-related, discover local candidates, update the cache, retry the original command, and protect it with a root `/.codex/` ignore rule. |
...
- Before toolchain commands, read stack-specific project configuration and validate the active cache path: write `.codex/local-environment.<profile>.json` by default and read legacy `.codex/local-environment.json` only as a fallback. Java/Maven uses `pom.xml`, `.mvn/*`, wrapper, and Java/Maven version constraints; Python uses `pyproject.toml`, lock files, requirements, tox/nox, and virtual environments; frontend uses the target `package.json`, lockfile, `engines`, `packageManager`, and scripts; Mini Programs use `project.config.json`, `app.json`, `pages.json`, `manifest.json`, Taro/uni-app config, and any required `package.json`.
...
- Before frontend compilation, builds, lint, format, type checks, or tests, read the target `package.json` scripts, dependencies, `engines`, `packageManager`, lockfile, and syntax/indentation/format configuration files such as ESLint, Prettier, EditorConfig, Biome, Stylelint, and TypeScript configs. Match the Node version, package manager, and project scripts, then write them to the active cache path under `frontendQuality`; reuse the cache only when it still matches the current package, command, and config-file set. If compilation fails and the failure may be caused by environment, dependency version, script, or config mismatch, reread project config and local tools, update the cache, and retry once.
...
- For builds, compilation, tests, runs, previews, or pre-release checks, first identify the workspace root and resolve the active cache path; use `.codex/local-environment.<profile>.json` by default and legacy `.codex/local-environment.json` only as a fallback.
