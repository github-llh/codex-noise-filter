...
- 将最新目标、不可违反约束、已确认事实、已改文件、验证状态、`doNotRetry` 和唯一下一步视为连续性账本；只在语义发生变化时更新，不按工具次数重复输出。
...
任务跨多个阶段、工具可能长时间运行或状态无法从单个 diff 恢复时维护短检查点；确认关键约束、完成写入、验证结果变化或准备长时间工具前按语义更新。压缩、恢复、模型/工作目录变化或网络/传输失败后自动读取 [10-continuity.md](references/10-continuity.md) 并执行复核；不要按固定文件数或工具调用次数输出模板。
