...
description: |
  ...
  识别到代码片段、报错日志、堆栈、构建/测试失败、路径、类名、方法名、配置文件、截图里的代码或错误、Java 后端 Controller/Service/DTO/Entity、前端导出类型/API 边界、Maven 多模块、事务并发、Python、Vue/React、小程序、文件归属、环境命令、local-environment 缓存、.codex 忽略规则、验证策略、安全边界、注释契约、编码风格智能化、魔法值、常量放置、硬编码、重复逻辑、Plan/Goal、上下文压缩、减少 token 或调用链确认等编程任务时自动启用。
...
- 代码片段、IDE 截图、diff、lint/typecheck 输出、阅读或检索命中的文件中已经出现明显硬编码、重复 if/set、外部协议常量、平台编码、配置写死、分层错位、导出接口/API 边界缺少必要注释或旧代码继续违背本 skill 规则。
