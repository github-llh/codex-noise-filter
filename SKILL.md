...
description: 编程任务的证据驱动执行、去噪、受控委派与连续性保护工作流。用于在现有代码库中进行代码读取、调试、修复、重构、构建、测试、lint、typecheck、代码审查、CI 诊断，以及 skill、plugin、agent 配置安全审计时，收敛文件范围、追踪根因和必要调用链、保护脏工作区并完成最小充分验证；按任务独立性、上下文成本、写冲突和预期收益判断是否使用单层子智能体；上下文压缩、会话恢复、模型切换或网络/传输失败后自动复核目标与落盘状态。出现代码、diff、路径、错误日志、异常堆栈、构建或测试输出、配置文件和项目结构时使用；纯知识问答、没有仓库上下文的泛泛建议、单纯翻译或改写不使用。
---
...
- 在任务边界首次明确或任务结构实质变化时做一次轻量工作量判断；不得按工具调用次数反复评估，也不得为了判断是否需要子智能体而先启动子智能体。只有存在可独立、可验收且收益高于协调与总 token 成本的工作通道时才最小委派，否则由主智能体直接完成。
- 主智能体是唯一调度者并负责计划、验收、冲突处理、最终写入与验证。此 skill 发起的委派只允许一层；子智能体若继承本 skill，执行已分配通道但跳过委派评估，不得继续派生。主智能体不得与运行中的子智能体重复处理同一范围。
...
- 将最新目标与验收条件、不可违反约束、已确认事实与关键决策、已改文件、验证状态、`doNotRetry` 和唯一下一步视为连续性账本；只在语义发生变化时更新，不按工具次数重复输出。Plugin Hook 在压缩前标记待恢复，并在压缩后的受支持事件中注入结构化账本复核。
- 检测到上下文压缩、会话恢复、模型切换、工作目录变化或网络/传输失败后，自动进入恢复流程，不要求用户重复信息，也不直接重试可能已有副作用的操作。
...
- skill 本身不能保证每个宿主都支持生命周期事件；Plugin 分发版在 Hook 已启用并完成首次信任后，使用内置连续性 Hook 自动处理已支持事件。Hook 被宿主禁用、管理员仅允许 managed hooks 或当前表面不支持事件时，退回本节的指令级恢复流程。

## 长任务与恢复

任务跨多个阶段、工具可能长时间运行或状态无法从单个 diff 恢复时维护短检查点；确认关键约束、完成写入、验证结果变化或准备长时间工具前按语义更新。压缩前确保摘要保留连续性账本，恢复后用当前工作区重新验证易过期事实。压缩、恢复、模型/工作目录变化或网络/传输失败后自动读取 [10-continuity.md](references/10-continuity.md) 并执行复核；不要按固定文件数或工具调用次数输出模板。
...
- 自动连续性保护：[10-continuity.md](references/10-continuity.md)
- Skill 与 Plugin 分发：[11-skill-and-plugin-distribution.md](references/11-skill-and-plugin-distribution.md)
- 资料调研与子智能体委派：[12-research-and-delegation.md](references/12-research-and-delegation.md)
