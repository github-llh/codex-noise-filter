...
description: 编程任务的证据驱动执行、去噪与连续性保护工作流。用于在现有代码库中进行代码读取、调试、修复、重构、构建、测试、lint、typecheck、代码审查、CI 诊断，以及 skill、plugin、agent 配置安全审计时，收敛文件范围、追踪根因和必要调用链、保护脏工作区并完成最小充分验证；上下文压缩、会话恢复、模型切换或网络/传输失败后自动复核目标与落盘状态。出现代码、diff、路径、错误日志、异常堆栈、构建或测试输出、配置文件和项目结构时使用；纯知识问答、没有仓库上下文的泛泛建议、单纯翻译或改写不使用。
---
