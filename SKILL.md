...
- 当前任务处于上个会话接续、上下文恢复、自动续跑、跨窗口继续、引用上一轮结论、存在 Context Capsule、或 skill/reference 近期在当前工作区发生变更的状态；这类状态必须自动刷新规则，不依赖用户提醒。
- 任务涉及路径、文件名、类名、方法名、错误日志、调用链、构建、测试、前后端代码规范。
- 用户要求提高写代码风格、智能化抽象、减少魔法值、抽常量、枚举化、配置化、去掉硬编码或避免重复逻辑。
