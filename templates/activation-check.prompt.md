...
判断时不要只看是否显式写出 skill 名，也不要受调用入口影响。任务来自任意第三方调用、App、终端/CLI、IDE 插件、MCP/ACP、hook、subagent、CI/chatops、`cc switch`、模型/供应商路由、未知 wrapper 或未来新增工具时，只要载荷包含代码、路径、命令、日志、diff、截图里的错误、项目结构、中文乱码、编码/charset 信号或工具链动作，就按本 skill 内部触发。
...
6. 禁止触碰范围。
7. 修改前必须闭环的调用链或影响面。
8. 是否需要任务胶囊/快照、局部对齐、抽象抽离、编码/中文乱码检查、环境缓存。
9. 最小验证项。
