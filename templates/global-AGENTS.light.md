...
2. 任务胶囊/快照：复杂任务开始、读取 3 个文件、执行 2 次工具调用、耗时工具调用前、上下文压缩前后、每次写入前、阶段完成后都输出或刷新 Capsule。
...
8. 恢复交付：网络断开、工具超时、上下文压缩、`PreCompact`/`PostCompact`/`SessionStart compact` 或模型切换后，先读当前文件、`git diff`、`git status` 和最近 Capsule，从断点继续；大段日志和搜索结果只保留关键错误、路径/行号、命令和结论。
