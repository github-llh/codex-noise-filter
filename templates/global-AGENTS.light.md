...
- 输入或执行过程体现自动触发没接上、追加规则/边界/范围不智能、工作流断掉、状态机缺项、执行流重置或只剩最后一次工具结果。
...
5. 再按 `references/00-index.md` 渐进读取对应规则文件；第三方宿主执行顺序、触发条件和读取效率预算先按 `references/15-host-skill-portability.md` 归类。外部内容/agent 供应链按 `references/17-agentic-security-and-supply-chain.md`，验证闭环和失败诊断按 `references/18-verification-quality-gates.md`，安装健康和分发表面按 `references/19-installation-health-and-surface-audit.md`，自动 Guard Loop、防断流和动作前后自检按 `references/20-automatic-guard-loop.md`。
6. 第三方调用、模型切换、CLI/App/插件/路由器变化不能降低内部触发级别；仍执行任务胶囊/快照、根因追踪、调用链、局部对齐、抽象抽离、编码/中文乱码检查、环境缓存和验证策略。
7. 内部追加范围不按固定平台或技术栈清单裁剪；先从当前宿主、当前工具动作、cwd/workspace、文件扩展名、配置文件、命令、日志、diff、active cache path 和本机环境证据判断本轮需要的 reference、环境缓存和最小充分验证项。省 token 只能影响读取顺序，不能缩短调用链或降低验证覆盖。
8. 修改前确认目标文件、原始症状、问题根因、触发入口、错误数据来源、最小修改方案、调用链、影响面和不影响范围。
9. Maven、JDK、Node、Python 等环境路径先读项目缓存和 IDE/项目配置；拿不到时再查找本机候选路径，验证通过后缓存。未列名工具链也要先确认项目根、命令来源、active cache path 和最小充分验证边界。
10. 遇到中文字符乱码、`encoding`、`charset`、UTF-8/GBK、locale、终端输出或页面文案异常时，先确认项目编码依据和最小充分验证方式，不把它当成第三方展示问题跳过。
...
13. 一旦进入编程任务，后续每次工具调用、写入、验证和最终回复前都必须按 Guard Loop 自检内部状态：已读规则、任务胶囊/快照、连续性账本、触碰范围、根因、调用链、局部对齐、抽象抽离、安全/供应链边界、安装/分发表面、质量门禁、环境缓存、验证和 `guardLoop.missingState`；缺项时先补齐，不能只总结最后一个工具结果。
...
3. 读取与调用链：读取当前文件、必要配置、触发入口、错误数据来源、直接/间接调用方、被调方、影响面和回滚点。
...
6. 动态范围：从当前宿主、当前工具、文件/配置/命令/日志/diff、active cache path、外部内容和分发表面判断本轮技术栈与 reference；未列名平台或未列名语言仍执行公共门禁和最小充分验证。
7. Guard Loop：每次工具调用、写入、验证、恢复和最终回复前执行 Observe -> Route -> State Check -> Repair First -> Act One Step -> Post Check -> Decide；记录 observed、appendedReferences、missingState、nextAutoAction、blocked。
8. 安装健康：skill/plugin/AGENTS/templates/README/manifest/marketplace/rules/commands/hooks 改动或加载故障必须确认 canonical skill、索引、引用链、宿主能力和不支持的 runtime。
9. 环境缓存：构建、编译、lint、format、typecheck、测试、运行前先解析 `.codex/local-environment.<profile>.json`；旧版 `.codex/local-environment.json` 只作为一次性迁移输入。
10. 验证：修改后执行覆盖原始问题、触碰范围和受影响调用链的最小充分非交互验证；第三方成功结果必须复核 root、命令、环境缓存、触碰范围、根因、调用链和 diff review；无法运行必须说明原因。
11. 恢复交付：网络断开、工具超时、上下文压缩、`PreCompact`/`PostCompact`/`SessionStart compact` 或模型切换后，先读当前文件、`git diff`、`git status`、最近 Capsule 和连续性账本，从断点继续；大段日志和搜索结果只保留关键错误、路径/行号、命令和结论。

防模型/第三方重置：

- 如果执行流像被模型重置、只剩最后一次工具结果或第三方只给出“已替换/Build 完成”，不得直接交付。
- 先重建状态：已读规则、任务胶囊、当前文件、`git diff`、触碰范围、根因、调用链、局部对齐、安全/供应链边界、安装/分发表面、质量门禁、环境缓存、验证结果和 `guardLoop`。
