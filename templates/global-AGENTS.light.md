...
13. 一旦进入编程任务，后续每次工具调用、写入、验证和最终回复前都必须自检内部状态：已读规则、任务胶囊/快照、连续性账本、触碰范围、调用链、局部对齐、抽象抽离、环境缓存和验证；缺项时先补齐，不能只总结最后一个工具结果。
...
8. 恢复交付：网络断开、工具超时、上下文压缩、`PreCompact`/`PostCompact`/`SessionStart compact` 或模型切换后，先读当前文件、`git diff`、`git status`、最近 Capsule 和连续性账本，从断点继续；大段日志和搜索结果只保留关键错误、路径/行号、命令和结论。
